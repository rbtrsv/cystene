...
import { Receipt, Plus, Loader2, ChevronUp, ChevronDown, ChevronsUpDown, Check, MoreHorizontal, Eye, Pencil } from 'lucide-react';
...
/**
 * Sort field options for the income statements table
 */
type SortField = 'year' | 'revenue' | 'net_income' | 'ebitda';
type SortDirection = 'asc' | 'desc' | null;

export default function IncomeStatementsPage() {
  const router = useRouter();
  const { activeEntity, getEntityName } = useEntities();
  const { incomeStatements, isLoading, error, fetchIncomeStatements } = useIncomeStatements();
  ...
  // Fetch all accessible income statements (backend filters by user entity access)
  useEffect(() => {
    if (activeEntity) {
  ...
  // Build entity options for the filter dropdown from entities that have income statements
  const entityFilterOptions = useMemo(() => {
    const entityIds = [...new Set(incomeStatements.map((item) => item.entity_id))];
    return entityIds.map((id) => ({
      id,
      name: getEntityName(id),
    })).sort((a, b) => a.name.localeCompare(b.name));
  }, [incomeStatements, getEntityName]);

  // ==========================================
  // Filter and sort income statements
  // ==========================================

  const filteredRows = useMemo(() => {
    // Start with all accessible income statements (backend already filters by user access)
    let filtered = [...incomeStatements];
      ...
      filtered = [...filtered].sort((a, b) => {
        ...
        switch (sortField) {
          ...
          case 'revenue':
            ...
            break;
          case 'net_income':
            comparison = (a.net_income ?? 0) - (b.net_income ?? 0);
            break;
          case 'ebitda':
            comparison = (a.ebitda ?? 0) - (b.ebitda ?? 0);
            break;
        }
    ...
    return filtered;
  }, [incomeStatements, entityFilter, scenarioFilter, yearFilter, searchTerm, sortField, sortDirection, getEntityName]);
  ...
  // Total count before filters (all accessible income statements)
  const totalCount = incomeStatements.length;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage income statement records across all accessible entities
          </p>
        </div>
        <Link href="/income-statements/new">
          <Button>
          ...
          {filteredRows.length === 0 ? (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <h3 className="text-lg font-semibold mb-2">
                  {totalCount === 0 ? 'No income statements yet' : 'No income statements match filters'}
                </h3>
                <p className="text-sm text-muted-foreground mb-4">
                  {totalCount === 0
                    ? 'Create your first income statement to track revenue and earnings'
            ...
            <>
                ...
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>
                      ...
                      >
                        Net Income
                        <SortIndicator field="net_income" />
                      </Button>
                    </TableHead>
                    <TableHead>
                      <Button
                ...
                <TableBody>
                  {filteredRows.map((item) => (
                    <TableRow
                      ...
                      onClick={() => router.push(`/income-statements/${item.id}/details`)}
                    >
                      ...
                      <TableCell>{fmtCurrency(item.revenue)}</TableCell>
                      <TableCell>{fmtCurrency(item.net_income)}</TableCell>
                      <TableCell>{fmtCurrency(item.ebitda)}</TableCell>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem
                              onClick={(e) => {
                                e.stopPropagation();
                                router.push(`/income-statements/${item.id}/details`);
                              }}
                            >
                            ...
                            <DropdownMenuItem
                              onClick={(e) => {
                                e.stopPropagation();
                                router.push(`/income-statements/${item.id}/details?tab=edit`);
                              }}
                            >
              ...
              <p className="text-sm text-muted-foreground">
                Showing {filteredRows.length} of {totalCount} income statements
              </p>
            </>
          )}
        </div>
