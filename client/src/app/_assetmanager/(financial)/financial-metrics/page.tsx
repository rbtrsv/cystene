...
/**
 * Sort field options for the financial metrics table
 */
type SortField = 'year' | 'arr' | 'burn_rate';
type SortDirection = 'asc' | 'desc' | null;

export default function FinancialMetricsPage() {
  const router = useRouter();
  const { activeEntity, getEntityName } = useEntities();
  const { financialMetrics, isLoading, error, fetchFinancialMetricsList } = useFinancialMetricsList();
  ...
  // Fetch all accessible financial metrics (backend filters by user entity access)
  useEffect(() => {
    if (activeEntity) {
  ...
  // Build entity options for the filter dropdown from entities that have financial metrics
  const entityFilterOptions = useMemo(() => {
    const entityIds = [...new Set(financialMetrics.map((item) => item.entity_id))];
    return entityIds.map((id) => ({
      id,
      name: getEntityName(id),
    })).sort((a, b) => a.name.localeCompare(b.name));
  }, [financialMetrics, getEntityName]);

  // ==========================================
  // Filter and sort financial metrics
  // ==========================================

  const filteredRows = useMemo(() => {
    // Start with all accessible financial metrics (backend already filters by user access)
    let filtered = [...financialMetrics];
      ...
      filtered = [...filtered].sort((a, b) => {
        ...
        switch (sortField) {
          ...
          case 'arr':
            ...
            break;
          case 'burn_rate':
            comparison = (a.burn_rate ?? 0) - (b.burn_rate ?? 0);
            break;
        }
    ...
    return filtered;
  }, [financialMetrics, entityFilter, scenarioFilter, yearFilter, searchTerm, sortField, sortDirection, getEntityName]);
  ...
  // Total count before filters (all accessible financial metrics)
  const totalCount = financialMetrics.length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Financial Metrics</h1>
          <p className="text-muted-foreground">
            Manage financial metrics records across all accessible entities
          </p>
        </div>
        <Link href="/financial-metrics/new">
          <Button>
          ...
          {filteredRows.length === 0 ? (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <h3 className="text-lg font-semibold mb-2">
                  {totalCount === 0 ? 'No financial metrics yet' : 'No financial metrics match filters'}
                </h3>
                <p className="text-sm text-muted-foreground mb-4">
                  {totalCount === 0
                    ? 'Create your first financial metrics record to track ratios, revenue, and operational KPIs'
            ...
            <>
                ...
                <TableBody>
                  {filteredRows.map((item) => (
                    <TableRow
                      ...
                      onClick={() => router.push(`/financial-metrics/${item.id}/details`)}
                    >
                      ...
                      <TableCell>{fmtCurrency(item.arr)}</TableCell>
                      <TableCell>{fmtCurrency(item.burn_rate)}</TableCell>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem
                              onClick={(e) => {
                                e.stopPropagation();
                                router.push(`/financial-metrics/${item.id}/details`);
                              }}
                            >
                            ...
                            <DropdownMenuItem
                              onClick={(e) => {
                                e.stopPropagation();
                                router.push(`/financial-metrics/${item.id}/details?tab=edit`);
                              }}
                            >
              ...
              <p className="text-sm text-muted-foreground">
                Showing {filteredRows.length} of {totalCount} financial metrics
              </p>
            </>
          )}
        </div>
