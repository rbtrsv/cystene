"""
Cybersecurity Utils — API Rate Limiting
...
Public API:
    ...
    - rate_limiter — global backend instance
"""
...
async def check_rate_limit(org_id: int, tier: str) -> None:
    ...
    minute_ok = await rate_limiter.check_and_increment(
        f"rl:cyber:{org_id}:min", limits["requests_per_minute"], 60
    )
    if not minute_ok:
        raise HTTPException(
    ...
    hour_ok = await rate_limiter.check_and_increment(
        f"rl:cyber:{org_id}:hr", limits["requests_per_hour"], 3600
    )
    if not hour_ok:
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
